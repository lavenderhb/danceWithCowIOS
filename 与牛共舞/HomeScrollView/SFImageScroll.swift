...
class SFImageScroll: UIView {
    ...
    func ShowImageScrollWithImageArray(array:NSArray,btnClick:BtnClickBlock,showStyle:ImageShowStayle) -> Void {
        ...
        pageView?.currentPageIndicatorTintColor = UIColor.init(red: 240 / 255.0, green: 42 / 255.0, blue: 57 / 255.0, alpha: 1.0)
        pageView?.pageIndicatorTintColor = UIColor.init(red: 117 / 255.0, green: 172 / 255.0, blue: 227 / 255.0, alpha: 1.0)
    ...
    // MARK 网络图片处理   nw_connection event, should deliver: true

    func networkImageSettingWithString(str:NSString,btn:UIButton) -> Void{
        //         let paths:NSString = NSHomeDirectory().stringByAppendingString("Library/Caches/Image")
//        let paths:NSString = NSHomeDirectory().stringByAppendingString("Library/Image")
