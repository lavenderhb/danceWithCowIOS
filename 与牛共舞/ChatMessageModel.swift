...
class ChatMessageModel:NSObject {
    ...
    func setChatModel(message:JMSGMessage!) {
            ...
            (message.content as! JMSGImageContent).thumbImageData({ (data, objectId, error) -> Void in
                if error == nil {
                    ...
                    if img?.size.height >= img?.size.width {
                        // 135
                        imgHeight = CGFloat(300)
                        imgWidth = (img?.size.width)!/(img?.size.height)! * imgHeight! + 100
                        imgWidth = (imgWidth < 55) ? 55 : imgWidth
                    } else {
                        imgWidth = CGFloat(300)
                        imgHeight = (img?.size.height)!/(img?.size.width)! * imgWidth! + 100
