...
class MainWebViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        SearchBuuton.addTarget(self, action
            : #selector(MoneyLSWebViewView.SearchAction), forControlEvents: UIControlEvents.TouchUpInside)
        headerView.addSubview(SearchBuuton)
        
        self.moneyLSWeb = UIWebView()
        moneyLSWeb?.frame = CGRectMake(0, 64, self.view.bounds.width, self.view.bounds.height - 84)
        moneyLSWeb?.backgroundColor = UIColor.whiteColor()
        self.view.addSubview(moneyLSWeb!)
        
        Backbutton = UIButton(frame: CGRectMake(0, 30, 20, 30))
