...
//var  chattingVC : JChatChattingViewController;

@objc(DMJChatChattingViewController)
class DMJChatChattingViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        self.navigationController?.navigationBar.barTintColor = UIColor.init(red: 246 / 255.0, green: 75 / 255.0 , blue: 22 / 255.0, alpha: 1.0)
        let  pushJudge = NSUserDefaults.standardUserDefaults()
        
        // (targetStr?.isEqualToString("notice")) != nil
        if ((pushJudge.objectForKey("push")?.isEqualToString("push")) != nil){
            
            let nac = UIBarButtonItem.init(image: UIImage(named: "22.jpg"), style: UIBarButtonItemStyle.Plain, target: self, action: #selector(DMJChatChattingViewController.rebackToRootViewAction))
            
            
            self.navigationItem.leftBarButtonItem = nac
        }else{
        ...
        let gesture = UITapGestureRecognizer(target: self, action:#selector(DMJChatChattingViewController.handleTap(_:)))
    ...
    func setupNavigation() {
        ...
        self.title = LoadAll!["jiguang_kefu_nickname"] as? String
        ...
        backbb.addTarget(self, action: #selector(DMJChatChattingViewController.backToPrevious), forControlEvents: UIControlEvents.TouchUpInside)
    ...
    //    func backClick() {
    ////        for (self.navigationController?.vie)
    ////        这句话没断?
    //        self.navigationController?.popoverPresentationController
    //    }
    
    func clickRightBtn() {
        //        //  JCHATGroupDetailViewController
        //        let detailCtl = TopServiceWebView()
    ...
    override func viewWillAppear(animated: Bool) {
        ...
        NSNotificationCenter.defaultCenter().addObserver(self, selector:NSSelectorFromString("tiao:") , name: "tiao", object: nil)
...
extension DMJChatChattingViewController:UITableViewDelegate,UITableViewDataSource {
...
extension DMJChatChattingViewController:UIGestureRecognizerDelegate {
    func handleTap(recognizer: UITapGestureRecognizer) {
        DMhideKeyBoardAnimation()
...
extension DMJChatChattingViewController:JChatInputViewDelegate {
    ...
    func appendMessage(model:JChatMessageModel) {
        ...
        self.performSelector(#selector(DMJChatChattingViewController.appendMessageCell), withObject: nil, afterDelay: 0.01);
    }
...
// TODO:
extension DMJChatChattingViewController : JChatMessageCellDelegate {
...
extension DMJChatChattingViewController: JMessageDelegate {
