  ...
  func alertView(alertView: UIAlertView, clickedButtonAtIndex buttonIndex: Int) {
    ...
    if alertView.textFieldAtIndex(0)?.text == "" {
      ...
      MBProgressHUD.showMessage("正在创建单聊", toView: self.view)
        ...
        // 这个地方是输入的好友的名字 (alertView.textFieldAtIndex(0)?.text)!
      JMSGConversation.createSingleConversationWithUsername("ytt1226", completionHandler: { (singleConversation, error) -> Void in
