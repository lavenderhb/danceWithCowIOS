...
class ChatChattingDataSource: NSObject {
    ...
    init(showTimeInterval:Double, fristPageNumber:Int, limit:Int) {
        super.init()
    ...
    func appendMessage(model:NSObject) {
        if model.isKindOfClass(JChattimeModel) {
            ...
            self.getMessageCellHeight(model as! ChatMessageModel)
            self.allMessageDic.setObject(model, forKey: (model as! ChatMessageModel).message.msgId)
            
            self.allMessageIdArr.addObject((model as! ChatMessageModel).message.msgId)
...
//        arrList.addObjectsFromArray((self.conversation.messageArrayFromNewestWithOffset(self.messageOffset, limit: self.messagefristPageNumber) as NSArray).reverseObjectEnumerator().allObjects)
        ...
        for (_, value) in arrList.enumerate() {
            ...
            let model:ChatMessageModel = ChatMessageModel()
...
//        arrList.addObjectsFromArray(self.conversation.messageArrayFromNewestWithOffset(self.messageOffset, limit: self.messageLimit) as NSArray as [AnyObject])
    ...
    func getMessageWithIndex(index:Int) -> AnyObject {
        ...
        if messageId.isKindOfClass(JChattimeModel) {
            ...
            let model:ChatMessageModel = self.allMessageDic.objectForKey(messageId) as! ChatMessageModel
            return model
        }
    }
     ...
     *  通过msgId 获取指定消息model
     */
    func getMessageWithMsgId(messageId:String) -> ChatMessageModel{
        return self.allMessageDic.objectForKey(messageId) as! ChatMessageModel
    }
    ...
    internal func getMessageCellHeight(model: ChatMessageModel) -> CGFloat {
    ...
    internal func dataMessageShowtime(timeNumber:NSNumber) {
        if self.allMessageIdArr.count > 0 {
            if self.allMessageIdArr.lastObject!.isKindOfClass(NSString) {
                ...
                let lastModel = self.allMessageDic.objectForKey(messageId!) as! ChatMessageModel
                let timeInterVal = timeNumber.doubleValue
                if lastModel.isKindOfClass(ChatMessageModel) != false {
    ...
    func dataMessageShowTimeToTop(timeNumber:NSNumber) {
        ...
        if self.allMessageIdArr.count > 0 {
            ...
            if fristObj.isKindOfClass(NSString) {
                let fristModel = self.allMessageDic.objectForKey(fristObj) as! ChatMessageModel
                let timeInterVal = timeNumber.doubleValue
                
                if fristModel.isKindOfClass(ChatMessageModel) == true {
