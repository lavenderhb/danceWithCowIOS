...
protocol PZBBHelperDelegate: NSObjectProtocol {
    func PZBBViewReloadData()
}
class PZBBhelper: NSObject {

    
    //  数据源
    var dataArray = NSMutableArray()
    // 代理属性
    weak var delegate: PZBBHelperDelegate!
    // 创建单例
    class func moneysharedHelper() ->PZBBhelper{
        struct NetworkSTR{
            static var helper: PZBBhelper!
            static var onceToken: dispatch_once_t = 0
        }
        _dispatch_once(&NetworkSTR.onceToken) {
            // 这里代码只能执行一次
            NetworkSTR.helper = PZBBhelper()
        ...
        let dataTask = NSURLSession.sharedSession().dataTaskWithRequest(urlRequest) {(data: NSData?, response:NSURLResponse?, error:NSError?) -> Void in
            if error == nil {
                ...
                for aDict in itemsArray{
                    ...
                    let moneyTalkModel = PZBBModel(dict: modelDict)
                    // 将模型添加都数据源中
                ...
                // 刷新tableView
                // 如果代理存在，且响应协议中的方法
                if self.delegate != nil && self.delegate.respondsToSelector(Selector("PZBBViewReloadData")){
                    self.delegate.PZBBViewReloadData()
                }
