...
protocol FiveStareHelperDelegate: NSObjectProtocol {
    func FiveStareViewReloadData()
}


class FiveStareHelper: NSObject {

    //  数据源
    var dataArray = NSMutableArray()
    // 代理属性
    weak var delegate: FiveStareHelperDelegate!
    // 创建单例
    class func moneysharedHelper() ->FiveStareHelper{
        struct NetworkSTR{
            static var helper: FiveStareHelper!
            static var onceToken: dispatch_once_t = 0
        }
        _dispatch_once(&NetworkSTR.onceToken) {
            // 这里代码只能执行一次
            NetworkSTR.helper = FiveStareHelper()
...
//                    
//                    // 通过构造方式来创建模型
                    let moneyTalkModel = FiveStareModel(dict: modelDict)
                    // 将模型添加都数据源中
                ...
                // 刷新tableView
                // 如果代理存在，且响应协议中的方法
                if self.delegate != nil && self.delegate.respondsToSelector(Selector("FiveStareViewReloadData")){
                    self.delegate.FiveStareViewReloadData()
                }
