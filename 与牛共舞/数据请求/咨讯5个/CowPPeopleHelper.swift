...
protocol CowPPeopleHelperDelegate: NSObjectProtocol {
    func CowPPeopleViewReloadData()
}

class CowPPeopleHelper: NSObject {

    //  数据源
    var dataArray = NSMutableArray()
    // 代理属性
    weak var delegate: CowPPeopleHelperDelegate!
    // 创建单例
    class func moneysharedHelper() ->CowPPeopleHelper{
        struct NetworkSTR{
            static var helper: CowPPeopleHelper!
            static var onceToken: dispatch_once_t = 0
        }
        _dispatch_once(&NetworkSTR.onceToken) {
            // 这里代码只能执行一次
            NetworkSTR.helper = CowPPeopleHelper()
        ...
        let dataTask = NSURLSession.sharedSession().dataTaskWithRequest(urlRequest) {(data: NSData?, response:NSURLResponse?, error:NSError?) -> Void in
            if error == nil {
                ...
                for aDict in itemsArray{
                    ...
                    // 通过构造方式来创建模型
                    let moneyTalkModel = CowManModel(dict: modelDict)
                    // 将模型添加都数据源中
                ...
                // 刷新tableView
                // 如果代理存在，且响应协议中的方法
                if self.delegate != nil && self.delegate.respondsToSelector(Selector("CowPPeopleViewReloadData")){
                    self.delegate.CowPPeopleViewReloadData()
                }
