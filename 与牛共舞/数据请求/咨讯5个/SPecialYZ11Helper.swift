...
// swift中的协议
protocol SPecialYZ11HelperDelegate: NSObjectProtocol {
    func SPecialYZ11ViewReloadData()
}

class SPecialYZ11Helper: NSObject {

    //  数据源
    var dataArray = NSMutableArray()
    // 代理属性
    weak var delegate: SPecialYZ11HelperDelegate!
    // 创建单例
    class func moneysharedHelper() ->SPecialYZ11Helper{
        struct NetworkSTR{
            static var helper: SPecialYZ11Helper!
            static var onceToken: dispatch_once_t = 0
        }
        _dispatch_once(&NetworkSTR.onceToken) {
            // 这里代码只能执行一次
            NetworkSTR.helper = SPecialYZ11Helper()
        ...
        let dataTask = NSURLSession.sharedSession().dataTaskWithRequest(urlRequest) {(data: NSData?, response:NSURLResponse?, error:NSError?) -> Void in
            if error == nil {
                ...
                for aDict in itemsArray{
                    ...
                    // 通过构造方式来创建模型
                    let moneyTalkModel = SpecialModel(dict: modelDict)
                    // 将模型添加都数据源中
                ...
                // 刷新tableView
                // 如果代理存在，且响应协议中的方法
                if self.delegate != nil && self.delegate.respondsToSelector(Selector("SPecialYZ11ViewReloadData")){
                    self.delegate.SPecialYZ11ViewReloadData()
                }
