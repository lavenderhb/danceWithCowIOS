...
import UIKit
// swift 中的协议
protocol EveryThreeNetWorkHelperDelegate : NSObjectProtocol{
    func EveryThreetableViewReloadData()
}


class EveryThreeHelper: NSObject {
    // 注意,静态属性只能在类里面的进行声明,不能卸载方法里面
    
    // 数据源
    var dataArray = NSMutableArray()
    // 代理属性
    weak var delegate : EveryThreeNetWorkHelperDelegate!
    
    // 创建单例
    class func sharedHelper() -> EveryThreeHelper {
        struct NetWorkSTR{
            static var helper : EveryThreeHelper!
            static var onceToken : dispatch_once_t = 0
        }
        
        dispatch_once (&NetWorkSTR.onceToken){ () -> Void in
            // 在这里代码只会被执行一次
            NetWorkSTR.helper = EveryThreeHelper()
        }
        ...
        let dataTask = NSURLSession.sharedSession().dataTaskWithRequest(urlRequest) { (data : NSData?, respone : NSURLResponse?, error : NSError?) -> Void in
            ...
            if error == nil{
                ...
                for aDict in itemsArray{
                    ...
                    let FiveDeaModel = EveryThree(dict: modelDict)
                ...
                if self.delegate != nil && self.delegate.respondsToSelector(Selector("EveryThreetableViewReloadData")){
                    self.delegate.EveryThreetableViewReloadData()
                }
