...
class CHWButton: UIButton {
    ...
    convenience init(count: Int,frame: CGRect , color: UIColor?) {
        ...
        self.addLabel()
        super.addTarget(self, action: #selector(CHWButton.startCountDown), forControlEvents: UIControlEvents.TouchUpInside )
    }
    ...
    //    开启定时器
    func startCountDown() {
        self.timer = NSTimer.scheduledTimerWithTimeInterval(1, target: self, selector: #selector(CHWButton.countDown), userInfo: nil, repeats: true)
