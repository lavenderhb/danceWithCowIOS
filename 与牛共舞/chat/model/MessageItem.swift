...
class MessageItem {
    ...
    //构造文本消息体
    convenience init(body:NSString, user:ChatUserInfo, date:NSDate, mtype:ChatType) {
        ...
        let size =  body.boundingRectWithSize(CGSize(width: CGFloat(width), height: CGFloat(height)),
    ...
    //可以传入更多的自定义视图
    init(user:ChatUserInfo, date:NSDate, mtype:ChatType, view:UIView, insets:UIEdgeInsets) {
        self.view = view
        self.user = user
        self.date = date
        self.mtype = mtype
    ...
    //构造图片消息体
    convenience init(image:UIImage, user:ChatUserInfo,  date:NSDate, mtype:ChatType) {
        var size = image.size
        //等比缩放
        if (size.width > 220) {
